...
name: payflow-design
description: |
  Payflow Design Skill。用于生成或评审支付、计费、金融、交易流、本地生活、
  商户数字化经营、营销增长和服务运营类 UI。
  覆盖 H5、小程序、Web 后台；会先判断场景、平台、页面模式和信息密度，
  再选择 antd-mobile、antd-mini、antd、@ant-design/pro-components 与 Design Token。
  当用户明确提到 Payflow、支付结果、收银台、账单计费、订阅套餐、交易流水、
  商户后台、本地生活、附近商家、优惠券、会员权益、营销活动、经营看板、
  金融/支付后台或交易服务类产品时触发；泛泛说"做个页面"时先确认是否采用本规范。
user-invocable: true
---

# Payflow Design Skill

Payflow 指用户、资金、订单、权益、商户和运营动作之间的完整流转。本 Skill 面向支付、计费、金融、交易流、本地生活、商户经营和营销增长等高信任业务界面，帮助 coding agent 在写代码前先完成产品设计判断。
...
1. **谁在用？** 用户角色（消费者/会员/商户/运营/财务/客服/管理员）
2. **什么场景？** 用户刚做了什么、即将做什么
3. **核心目标？** 这个页面要解决什么问题
4. **情绪状态？** 用户此刻的感受（成功/受阻/中性/焦虑/犹豫）
5. **业务域？** 支付、计费、交易、经营、服务、营销还是后台管理

> **Checkpoint**: 以上 5 点已明确或有合理默认值后，才进入 Phase 1。
...
| 小程序 | 小程序 | `antd-mini` v3 |
| 后台、管理端、Web、PC、商户后台、经营看板 | Web | `antd` v5 + `@ant-design/pro-components` |
| 原型、低保真设计 | 按目标平台选择 | 同上 |
...
再读取 `references/design-philosophy.md`。核心哲学是 **轻、稳、可信、可转化**：内容优先，减少视觉噪音，在关键节点建立信任并推动下一步行动。

### 五大页面模式

| 模式 | 典型场景 | 视觉策略 |
|------|----------|----------|
| 情绪头部型 | 支付结果、签约成功、退款完成 | 大面积品牌渐变 + 情绪反馈 + 主操作 |
| 信息密集型 | 首页、经营看板、服务频道 | 头部认知锚点 + 卡片流 + 功能入口 |
| 轻量列表型 | 消息、我的、卡包、订单列表 | 浅色头部 + 列表内容 + 安静层级 |
| 沉浸暗色型 | 视频、图片内容、活动互动 | 暗背景 + 内容优先 + 克制操作 |
| 极简表单型 | 登录、搜索、签约、计费配置 | 留白 + 单任务聚焦 + 少装饰 |
