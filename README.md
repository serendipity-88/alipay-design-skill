...
让 coding agent 生成更像真实支付、金融、生活服务产品的界面，而不是只会把组件摆上去。

这个 Skill 不是一个组件库，也不是一份设计规范的搬运。它更像给 agent 准备的一套「产品设计判断力」：当你说“做一个转账成功页”“做一个交易查询后台”“做一个附近优惠商家页”时，agent 不应该立刻开始写卡片和按钮，而应该先判断用户刚经历了什么、页面要承接哪种情绪、信息密度应该多高、用移动端还是 Web 后台、哪些状态不能漏掉。

组件库解决的是“有什么组件”。这个 Skill 想解决的是“什么时候该用什么结构、什么层级、什么语气，以及最后交付出来的页面像不像一个可信的支付服务产品”。

## 为什么做这个

现在的 coding agent 已经很会写 React，也很会调用 `antd`、`antd-mobile` 这类组件库。但一到真实业务界面，它们经常会暴露出几个问题：
...
这个 Skill 把这些判断前置成工作流。它会要求 agent 先理解场景，再选择页面模式、组件体系、视觉 token 和代码模板，最后用检查脚本做一次交付前自检。

## 它适合谁

如果你经常让 agent 做支付、金融、生活服务、电商交易、运营承接、商户后台、管理端表单这类页面，这个 Skill 会很有用。
...
它不试图替代设计师，也不试图替代组件库。它的价值是把一部分高频、稳定、可复用的产品设计判断交给 agent，让第一次生成就更接近可评审的版本。

## 它怎么工作

Skill 被触发后，会按一条固定路径推进：
...
这套流程的重点不是让 agent “多问几句”，而是让它少犯低级判断错误：该克制的时候克制，该承接情绪的时候承接情绪，该补状态的时候补状态。

## 和直接使用 Ant Design 的差异

Ant Design、Ant Design Mobile、Ant Design Mini 和 Pro Components 提供了很好的组件与设计基础。这个 Skill 是面向 agent 的上层工作流：它把公开规范、组件习惯、页面模式和业务场景组织成可执行的生成过程。

简单说：

| 直接使用组件库 | 使用这个 Skill |
| --- | --- |
| 你告诉 agent 用哪个组件 | Skill 先帮 agent 判断页面应该怎么组织 |
